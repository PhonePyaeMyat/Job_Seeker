  ...
  const loadSavedJobs = async () => {
    ...
    try {
      ...
      if (userDoc.exists()) {
        ...
        const savedIds: string[] = userData.savedJobs || [];
        setSavedJobIds(savedIds);

        if (savedIds.length > 0) {
          // Fetch each saved job by ID to ensure presence regardless of current jobs query
          const fetchedSavedJobs: Job[] = [];
          for (const savedId of savedIds) {
            try {
              const jobSnap = await getDoc(doc(db, 'jobs', savedId));
              if (jobSnap.exists()) {
                const data = jobSnap.data() as any;
                fetchedSavedJobs.push({
                  id: jobSnap.id,
                  title: data.title || 'Untitled Job',
                  company: data.company || 'Unknown Company',
                  location: data.location || 'Location TBD',
              ...
              console.warn('Failed to fetch saved job', savedId, innerErr);
            }
          }
